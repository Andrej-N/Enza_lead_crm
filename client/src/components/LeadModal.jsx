    ...
    <div className="mb-3">
      ...
      {editing ? (
        <div className="flex gap-2 mt-1">
          <input
            className="flex-1 px-2 py-1 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
            value={val}
          ...
          <button onClick={() => { setVal(value || ''); setEditing(false) }} className="px-2 py-1 bg-gray-200 text-gray-600 rounded text-xs">X</button>
        </div>
      ) : (
        <div
          className="mt-1 text-sm text-gray-800 cursor-pointer hover:bg-gray-50 px-2 py-1 rounded border border-transparent hover:border-gray-200 transition min-h-6"
          onClick={() => setEditing(true)}
          title="Klikni za izmenu"
        >
          {value || <span className="text-gray-300 italic">Klikni za unos</span>}
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="mb-3">
      ...
      <div className="flex items-center gap-2 mt-1">
        <input
          type="date"
          className="flex-1 px-2 py-1.5 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white"
          value={val}
        ...
        {val && (
          <>
            ...
            <button onClick={clear} className="text-gray-400 hover:text-red-500 text-xs" title="Obrisi datum">✕</button>
          </>
        )}
      </div>
    </div>
  ...
  return (
    <div className="mb-3">
      ...
      {editing ? (
        <div className="mt-1">
          <textarea
            className="w-full px-2 py-1 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
            value={val}
            onChange={e => setVal(e.target.value)}
            rows={3}
            autoFocus
          ...
          <div className="flex gap-2 mt-1">
            ...
            <button onClick={() => { setVal(value || ''); setEditing(false) }} className="px-2 py-1 bg-gray-200 text-gray-600 rounded text-xs">Otkazi</button>
          </div>
        </div>
      ) : (
        <div
          className="mt-1 text-sm text-gray-800 cursor-pointer hover:bg-gray-50 px-2 py-1 rounded border border-transparent hover:border-gray-200 transition whitespace-pre-wrap min-h-6"
          onClick={() => setEditing(true)}
          title="Klikni za izmenu"
        >
          {value || <span className="text-gray-300 italic">Klikni za unos</span>}
...
export default function LeadModal({ lead, onClose, onUpdate, onDelete }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-start justify-end" onClick={onClose}>
      <div
        className="w-[500px] h-full bg-white shadow-2xl overflow-y-auto animate-slide-in"
      ...
      >
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between z-10">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className="text-xs text-gray-500">{lead.category === 'hotel' ? 'Hotel' : lead.category === 'klinika' ? 'Klinika' : lead.category === 'investitor' ? 'Investitor' : 'Prodavac'}</span>
              {lead.subcategory && <span className="text-xs text-gray-400">/ {lead.subcategory}</span>}
            </div>
          </div>
        ...
        <div className="px-6 py-4">
          {/* Status */}
          <div className="mb-6">
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Status Outreach</label>
              ...
              {Object.entries(STATUS_CONFIG).map(([k, v]) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium transition ${
                    lead.outreach_status === k ? v.bg + ' ring-2 ring-offset-1 ring-gray-300' : 'bg-gray-50 text-gray-400 hover:bg-gray-100'
                  }`}
                >
                  {v.label}
                </button>
          ...
          <div className="bg-gray-50 rounded-lg p-4 mb-6">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-gray-700">Kontakt informacije</h3>
              {/* Master verification badge */}
              {lead.phone_verified && lead.email_verified ? (
                <span className="px-2 py-0.5 bg-green-100 text-green-700 text-xs font-semibold rounded-full">Verifikovan</span>
              ) : null}
            </div>
            ...
            <div className="mt-4 pt-3 border-t border-gray-200">
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Verifikacija kontakta</label>
              <div className="mt-2 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Telefon verifikovan</span>
                  <button
                    onClick={() => onUpdate({ phone_verified: lead.phone_verified ? 0 : 1 })}
                    className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${lead.phone_verified ? 'bg-green-500' : 'bg-gray-300'}`}
                  >
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Email verifikovan</span>
                  <button
                    onClick={() => onUpdate({ email_verified: lead.email_verified ? 0 : 1 })}
                    className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${lead.email_verified ? 'bg-green-500' : 'bg-gray-300'}`}
                  >
                ...
                <div className="flex items-center justify-between pt-1 border-t border-gray-100">
                  <span className="text-sm font-medium text-gray-700">Sve verifikovano</span>
                  <button
                    onClick={() => {
                      const newVal = (lead.phone_verified && lead.email_verified) ? 0 : 1
                      onUpdate({ phone_verified: newVal, email_verified: newVal })
                    }}
                    className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${lead.phone_verified && lead.email_verified ? 'bg-green-600' : 'bg-gray-300'}`}
                  >
          ...
          {/* Location */}
          <div className="bg-gray-50 rounded-lg p-4 mb-6">
            <h3 className="text-sm font-semibold text-gray-700 mb-3">Lokacija</h3>
          ...
          {lead.category === 'hotel' && (
            <div className="bg-blue-50 rounded-lg p-4 mb-6">
              <h3 className="text-sm font-semibold text-blue-700 mb-3">Hotel detalji</h3>
          ...
          {lead.category === 'klinika' && (
            <div className="bg-rose-50 rounded-lg p-4 mb-6">
              <h3 className="text-sm font-semibold text-rose-700 mb-3">Klinika detalji</h3>
          ...
          {lead.category === 'investitor' && (
            <div className="bg-orange-50 rounded-lg p-4 mb-6">
              <h3 className="text-sm font-semibold text-orange-700 mb-3">Investitor detalji</h3>
              ...
              <div className="mt-2">
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Velicina</label>
                <select
          ...
          <div className="bg-emerald-50 rounded-lg p-4 mb-6">
            <h3 className="text-sm font-semibold text-emerald-700 mb-3">Pracenje kontakta</h3>
